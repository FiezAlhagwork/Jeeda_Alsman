...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  return (
    <div
      className={`sm-scope z-[9999] ${isFixed ? 'fixed top-0 left-0 w-full h-full pointer-events-none' : 'relative w-full h-full'}`}
    >
      <div
        className={
      ...
      >
        ...
        <header
          className="staggered-menu-header fixed top-8 left-1/2 -translate-x-1/2 w-auto pointer-events-none z-50"
          aria-label="Main navigation header"
        >
          <div 
            className={`flex items-center gap-8 px-8 py-4 rounded-full border border-white/10 backdrop-blur-xl transition-all duration-500 pointer-events-auto ${
          ...
          >
            <div className="sm-logo flex items-center select-none" aria-label="Logo">
              {logoUrl ? (
                ...
                <span className="text-xl font-black text-white tracking-tighter uppercase">{logoText}</span>
              )}
            </div>
            ...
            <button
              ref={toggleBtnRef}
              className="sm-toggle relative flex items-center gap-4 bg-transparent border-0 cursor-pointer text-[#e9e9ef] font-medium leading-none overflow-visible transition-all duration-300"
